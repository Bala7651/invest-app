        ...
        <View style={{ flexDirection: 'row', gap: 12, flexWrap: 'wrap' }}>
          {hasUpper ? (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                ...
                ↑{alert.upper_price} {alert.upper_status === 'active' ? t('alerts.active') : t('alerts.triggered')}
              </Text>
              {alert.upper_status === 'triggered' ? (
                <Pressable onPress={() => reEnable(alert.symbol, 'upper')}>
                  <Text style={{ color: '#00e5ff', fontSize: 11 }}>{t('alerts.reEnable')}</Text>
                </Pressable>
              ) : null}
            </View>
          ) : null}
          {hasLower ? (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                ...
                ↓{alert.lower_price} {alert.lower_status === 'active' ? t('alerts.active') : t('alerts.triggered')}
              </Text>
      ...
      >
        <Pressable onPress={() => {}} style={{ backgroundColor: '#0d0d14', borderTopWidth: 1, borderTopColor: '#2a2a3a', borderRadius: 16, padding: 24, paddingBottom: 24 + insets.bottom, maxHeight: '80%' }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <Text style={{ color: '#e0e0e0', fontSize: 18, fontWeight: 'bold' }}>{t('alerts.title')}</Text>
          ...
          {alerts.length === 0 ? (
            <Text style={{ color: '#555', fontSize: 14, textAlign: 'center', paddingVertical: 32 }}>{t('alerts.none')}</Text>
          ) : (
            <>
              {active.length > 0 ? (
                <>
                  <Text style={{ color: '#888', fontSize: 11, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>{t('alerts.active')}</Text>
                  {active.map(a => <AlertRow key={a.id} alert={a} />)}
                </>
              ) : null}
              {triggered.length > 0 ? (
                <View style={{ opacity: 0.5 }}>
                  <Text style={{ color: '#888', fontSize: 11, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>{t('alerts.triggered')}</Text>
