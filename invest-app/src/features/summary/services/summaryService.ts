...
export function formatSummaryError(error: unknown, language: AppLanguage = 'zh-TW'): string {
  const rawMessage = error instanceof Error ? error.message : String(error);
  const message = rawMessage.replace(/^Error:\s*/, '').trim();

  if (message.includes('AbortError')) {
    return language === 'en' ? 'AI request timed out. Please try again shortly.' : 'AI 請求逾時，請稍後重試';
  }
  ...
  if (message.startsWith('AI request failed') || message.startsWith('AI returned an empty response')) {
    return message;
  }
  return message || (language === 'en' ? 'Failed to generate the summary' : '摘要生成失敗');
}
...
function getSummarySystemPrompt(language: AppLanguage): string {
  if (language === 'en') {
    return `You are a professional Taiwan stock market analyst responsible for writing short daily market and stock summaries.

[Format rules]
1. Write entirely in English.
2. Output plain text only. No JSON, Markdown, headings, or special formatting.
3. Use 4-6 short sentences. Keep each sentence concise and direct.
4. Make the sentences flow as one coherent paragraph.

[Content structure]
Sentence 1: Today's close and the direction/magnitude of the move.
Sentence 2: Intraday behavior inferred from the open, high, and low.
Sentence 3: Volume observation based on volume ratio or trading activity.
Sentence 4: Technical positioning relative to moving averages if provided.
Sentence 5-6: A short-term outlook and the key support/resistance or upcoming observation points.

[Style]
- Professional and calm, like a senior analyst in a morning briefing.
- Objective and balanced, with no sensational language.
- Only comment on the data provided by the user. Never invent missing figures or indicators.`;
  }

  return `你是一位專業的台灣股市分析師，負責撰寫每日精簡的盤勢與個股摘要，你將會搜尋網路來確定以下的內容架構。
...
export function buildSummaryPrompt(
  symbol: string,
  name: string,
  quote: SummaryQuoteData,
  language: AppLanguage = 'zh-TW',
): string {
  const sign = quote.change >= 0 ? '+' : '';
  if (language === 'en') {
    return `Generate a short daily summary for Taiwan stock ${symbol} (${name}).
Today's market data (use only the exact values below and do not invent anything):
- Current price: ${quote.price ?? 'No data'} TWD
- Change: ${sign}${quote.change.toFixed(2)} (${quote.changePct.toFixed(2)}%)
- Previous close: ${quote.prevClose} TWD
Use 2-3 sentences to explain today's price action, key technical signals, and the short-term outlook.`;
  }
...
export function buildIndexSummaryPrompt(
  indexData: { close: number; change: number; changePct: number },
  language: AppLanguage = 'zh-TW',
): string {
  const sign = indexData.change >= 0 ? '+' : '';
  if (language === 'en') {
    return `Generate a short daily summary for the Taiwan Weighted Index.
Latest index data:
- Index: ${indexData.close.toFixed(2)} points
- Change: ${sign}${indexData.change.toFixed(2)} points (${sign}${indexData.changePct.toFixed(2)}%)
Use 4-5 sentences to describe today's market performance, the key move, and the short-term outlook.`;
  }
...
export async function callSummaryMiniMax(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (language === 'en') {
        throw new Error(`AI request failed (HTTP ${res.status})${body ? `: ${body.slice(0, 80)}` : ''}`);
      }
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      throw new Error(language === 'en' ? 'AI request timed out. Please try again shortly.' : 'AI 請求逾時，請稍後重試');
    }
    throw error;
  } finally {
    clearTimeout(timer);
...
export async function generateSummaryContent(
  ...
  const firstRaw = await callSummaryMiniMax(symbol, userPrompt, credentials, language);
  const firstContent = sanitizeSummaryContent(firstRaw);
  if (firstContent) return firstContent;

  const retryRaw = await callSummaryMiniMax(symbol, userPrompt, credentials, language);
  const retryContent = sanitizeSummaryContent(retryRaw);
  if (retryContent) return retryContent;

  throw new Error(
    language === 'en'
      ? 'AI returned an empty response after one automatic retry'
      : 'AI 回應為空（已自動重試 1 次）'
  );
}
